...
const Logo = ({ scrolled, theme }: { scrolled: boolean; theme?: string }) => {
  ...
  const defaultBg = isDark ? '#f8fafc' : '#1e293b';
  const defaultRed = isDark ? '#f87171' : '#dc2626';

  return (
    <a href="#hero" className="flex items-center gap-0.5 font-bold tracking-tight text-lg">
      <motion.span animate={{ color: scrolled ? '#ffffff' : defaultBg }} transition={{ duration: 0.3 }} className="font-script text-2xl font-normal leading-none">
        M
      </motion.span>
      <motion.span animate={{ color: scrolled ? '#111827' : defaultRed }} transition={{ duration: 0.3 }} className="font-script text-2xl font-normal leading-none">
        A
      </motion.span>
      <motion.span animate={{ color: scrolled ? '#ffffff' : defaultBg }} transition={{ duration: 0.3 }} className="font-script text-2xl font-normal leading-none">
        F
      </motion.span>
      <motion.span animate={{ color: scrolled ? '#111827' : defaultRed }} transition={{ duration: 0.3 }} className="font-bold text-lg leading-none">
        .
      </motion.span>
    </a>
  );
...
export default function Navbar() {
  ...
  return (
    <motion.header 
      ...
      className="fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 py-4 md:py-6 flex justify-center"
    >
      <nav className="w-full xl:w-fit max-w-full">
        ...
        >
          ...
          <motion.div className="flex-shrink-0">
            <Logo scrolled={scrolled} theme={mounted ? currentTheme : undefined} />
          </motion.div>
          ...
          <div className="flex flex-shrink-0 items-center gap-1 sm:gap-2 lg:border-l lg:border-slate-200 dark:lg:border-slate-700 lg:pl-6 lg:ml-2">
            ...
            <motion.button 
              onClick={() => {
                if (typeof document !== 'undefined' && 'startViewTransition' in document) {
                  ...
                  (document as any).startViewTransition(() => setTheme(isDark ? 'light' : 'dark'));
                } else {
            ...
            >
              {mounted ? (isDark ? <Sun className="w-4 h-4 md:w-5 md:h-5" /> : <Moon className="w-4 h-4 md:w-5 md:h-5" />) : <span className="w-4 h-4 md:w-5 md:h-5" />}
            </motion.button>
